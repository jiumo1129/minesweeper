...
- [x] 播放结束后自动切换到下一首（顺序循环）
- [x] 计时器模拟自动下一首（歌曲时长+3秒缓冲）
- [x] 迷你播放条添加上一首/下一首按钮，同步更新当前歌曲信息
- [x] 棋盘支持双指捏合缩放（PinchGestureHandler）
- [x] 缩放范围限制（最小0.5x，最大2.5x）
- [x] 缩放时配合平移，保持缩放中心点
- [x] 双击重置缩放到默认大小
- [x] 修复迷你播放条暂停按钮无效（用 isPlayingRef 解决 stale closure 问题）
- [x] 修复歌曲进度条读取错误（改为本地计时器模拟进度，显示 mm:ss/mm:ss）
- [x] 彻底修复迷你播放条暂停按钮无效（URL重载方案）
- [x] 修复游戏界面棋盘上方和下方的大块空白区域（ScrollView方案，棋盘自适应高度）
